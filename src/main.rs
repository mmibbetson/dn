...
use clap::Parser;
use cli::Cli;
use config::{load_config, read_config, Config};
use content::concatenate_file_content;
use directory::environment_config_dir;
    ...
    match &cli.command {
        cli::Commands::New {
            ...
            let config = {
                ...
                let config_base = match load_config(cli_config_path) {
                    Ok(config) => config,
                    Err(error) => {
        ...
        cli::Commands::Rename {
            ...
            let config = {
                ...
                // TODO: Probably best to be more explicit with these kinds of things...
                // if let Some(path) = config_base {
                //     config_builder = config_builder.with_base_config(&path);
                // };
